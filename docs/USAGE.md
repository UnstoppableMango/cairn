...
Every component on every machine moves together, etcd included: members run the etcd kubeadm ships for that minor, with `etcdctl` and `etcdutl` on their PATH from the same set, and the `kubectl` the kubeconfig service installs comes from the same minor.
