...
- `checks/addon-manifests.nix`: an evaluation-only flake check over the hand-authored addon manifests (`modules/service/*/manifests.nix`), called with fixtures rather than through inoculant, which would mean IFD over clan vars paths. The VM test bootstraps no addons and the option-tree check forces only the options, so nothing else covers the manifest bodies.
